...
- 配置「卡片模板 ID」后，Agent 执行过程实时推送到钉钉 AI 卡片，思考与输出边生成边展示
- 每轮 CG 交互一张卡片：提问时卡片定格为问题，回复后新卡片继续流式展示
- 未配置模板或卡片投放失败时，自动降级为 Markdown 消息，不影响使用
- 前置准备：钉钉开放平台「卡片平台」创建 AI 卡片模板（输入中/完成状态绑定 `content` 变量并开启流式），应用开通 `Card.Instance.Write` 与 `Card.Streaming.Write` 权限
